...
// GAP-I6 (v1.73): UI LAYER ONLY — do NOT import in services/, repositories/, or db/.
export function formatSKUDisplay(sku: string): string {
  ...
  const displaySeq = seqNum < 10 ? `0${seqNum}` : String(seqNum); // FIX-SKU-DISPLAY-2 (v1.51): min 2 digits
